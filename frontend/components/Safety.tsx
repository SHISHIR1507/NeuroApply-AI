  ...
  return (
    <section id="safety" ref={ref} style={{ padding: "120px 24px", maxWidth: 1100, margin: "0 auto" }}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} style={{ textAlign: "center", marginBottom: 26 }}>
        ...
        <h2 style={{ fontSize: "clamp(32px, 4vw, 52px)", fontWeight: 800, letterSpacing: "-0.03em", margin: "0 0 16px", background: "linear-gradient(135deg, #fafafa 0%, #86efac 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 18 }}>
        {POINTS.map((p, i) => (
          <motion.div key={p.title} initial={{ opacity: 0, y: 28 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: i * 0.1, duration: 0.55, ease: [0.22, 1, 0.36, 1] }}
            ...
            <h3 style={{ fontSize: 17, fontWeight: 700, color: "#fafafa", margin: "0 0 8px", letterSpacing: "-0.015em" }}>{p.title}</h3>
            <p style={{ fontSize: 14, color: "#52525b", lineHeight: 1.65, margin: 0 }}>{p.body}</p>
          </motion.div>
        ))}
      </div>
